...
public class JwtUtils {
    ...
    public static JwtToken createToken(String username, String role) {
        ...
        String token = Jwts.builder() // cria o token
                ...
                .signWith(getSecretKey()) //adiciona a chave secreta
    ...
    public static boolean isTokenValid(String token) {
        try {
            Jwts.parser()// faz a verificação do token
                    .verifyWith(getSecretKey())// verifica com a chave secreta
                    .build()
                    .parseSignedClaims(refactorToken(token));// faz o parse do token
            return true;
        } catch (JwtException ex) {
            log.error(String.format("Token invalido %s", ex.getMessage()));
        }
